package me.gabryon.ap_second_assignment;
...
public class TestAlgsPlus extends TestAlgs {
    ...
    protected static void testCiphers(KeyRegistry registry, List<String> testSet, List<Class<?>> classes)
        ...
        for (var clazz : classes) {
            ...
            /* 2. Does the class contain a method starting with `enc` with one string param or an annotated method with @Encrypt? */
            ...
            if (boxedEncMethod.isEmpty() && boxedAnnotEncMethod.isEmpty()) {
                System.out.printf("[Warning] The class '%s' doesn't have a method starting with 'enc' or annotated with @Encrypt. Skipped.\n", clazz.getName());
                continue;
            }

            /* 3. Does the class contain a method starting with `dec` with one string param or an annotated method with @Decrypt? */
            ...
            if (boxedDecMethod.isEmpty() && boxedAnnotDecMethod.isEmpty()) {
                System.out.printf("[Warning] The class '%s' doesn't have a method starting with 'dec' or annotated with @Decrypt. Skipped.\n", clazz.getName());
                continue;
            }
